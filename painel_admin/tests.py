...
class AlterarSenhaTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username='admin', password='velha-123')
        self.user.is_staff = True
        self.user.save()

    def test_requer_staff(self):
        comum = User.objects.create_user(username='comum', password='x')
        self.client.force_login(comum)
        resp = self.client.get('/painel/conta/senha/')
    ...
    def test_pagina_renderiza_form(self):
        ...
        self.assertContains(resp, 'Senha atual')
        self.assertContains(resp, 'Nova senha')

    def test_troca_senha_com_sucesso(self):
        self.client.force_login(self.user)
        resp = self.client.post('/painel/conta/senha/', {
